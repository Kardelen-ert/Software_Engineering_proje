...
def generate_recommendation(emotion, stress: float) -> list[str]:
    recommendations = []

    if emotion.anxiety > 0.5:
        recommendations.append(random.choice(ANXIETY_RECOMMENDATIONS))
    ...
    if emotion.anger > 0.5:
        recommendations.append(random.choice(ANGER_RECOMMENDATIONS))
